...
// ── CRTP mixin ──────────────────────────────────────────────────────────────
// Provides the shared read accessors (.source(), .tag(), .count()) for both
...
public:
    ...
    template <typename T>
        requires kamping::types::is_builtin_type_v<T>
    [[nodiscard]] int count() const {
        return count(kamping::types::builtin_type<T>::data_type());
    }
};
