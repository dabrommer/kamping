...
void alltoallv(SBuf&& sbuf, RBuf&& rbuf, Comm const& comm = MPI_COMM_WORLD) {
    int err = MPI_Alltoallv(
        ...
        std::ranges::data(kamping::ranges::displs(sbuf)),
...
auto alltoallv(SBuf&& sbuf, RBuf&& rbuf, Comm const& comm = MPI_COMM_WORLD) -> result<SBuf, RBuf> {
    ...
    infer(comm_op::alltoallv{}, res.send, res.recv, kamping::bridge::native_handle(comm));
    core::alltoallv(res.send, res.recv, comm);
    return res;
}
} // namespace kamping::v2
