...
class flatten_v_view
    ...
    void ensure_flattened() const {
        ...
        displs_.resize(num_ranks);
        std::exclusive_scan(counts_ptr, counts_ptr + num_ranks, displs_.begin(), 0);
...
public:
    ...
    flatten_v_view(S&& source, F&& flat_buf, C&& counts)
...
template <typename S, typename F, typename C>
flatten_v_view(S&&, F&&, C&&)
    -> flatten_v_view<kamping::ranges::all_t<S>, kamping::ranges::all_t<F>, kamping::ranges::all_t<C>>;

template <typename Source, typename FlatBuf, typename Counts, bool rb, bool rc>
inline constexpr bool enable_borrowed_buffer<flatten_v_view<Source, FlatBuf, Counts, rb, rc>> =
    enable_borrowed_buffer<FlatBuf> && enable_borrowed_buffer<Counts>;
...
inline constexpr struct flatten_v_fn {
    /// 0-arg: allocate flat buffer and counts internally, both auto-resized.
    constexpr auto operator()() const {
        return kamping::ranges::adaptor<0, decltype([](auto&& source) {
            ...
            return kamping::ranges::flatten_v_view<S, F, C, true, true>(
    ...
    /// 1-arg (non-resize): user flat buffer, internal counts (counts auto-resized).
    ...
    constexpr auto operator()(Fb&& flat_buf) const {
        return kamping::ranges::adaptor<1, decltype([](auto&& source, auto&& fb) {
            ...
            using C = kamping::ranges::owning_view<std::vector<int>>;
            return kamping::ranges::flatten_v_view<S, F, C, false, true>(
                std::forward<decltype(source)>(source),
    ...
    /// 2-arg (non-resize): user flat buffer + user counts (no resize for either).
    ...
    /// 1-arg (resize): user flat buffer with resize, internal counts (counts auto-resized).
    template <typename Fb>
    ...
    /// 2-arg (resize): user flat buffer + user counts (both resized).
    template <typename Fb, typename Ct>
