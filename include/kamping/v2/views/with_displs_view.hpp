...
template <typename R, typename C>
with_displs_view(R&&, C&&) -> with_displs_view<kamping::ranges::all_t<R>, kamping::ranges::all_t<C>>;

template <typename R, typename C>
with_displs_view(kamping::v2::monotonic_t, R&&, C&&)
    -> with_displs_view<kamping::ranges::all_t<R>, kamping::ranges::all_t<C>, true>;
...
inline constexpr struct with_displs_fn {
    ...
    // Uses adaptor<1> which distinguishes by count, not by type — safe for range displs args.
    template <typename... Args>
        requires(sizeof...(Args) >= 1
                 && !std::same_as<
                     std::remove_cvref_t<std::tuple_element_t<0, std::tuple<Args...>>>,
                     kamping::v2::monotonic_t>)
